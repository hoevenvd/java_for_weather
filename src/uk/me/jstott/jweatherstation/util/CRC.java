 ...
 */
public class CRC {
  private static final int[] TABLE = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084,
      0x50a5, 0x60c6, 0x70e7, // 0x00
  ...
  public void updateCRC(byte data) {
    updateCRC((int) data & 0xFF);
  }

  public void updateCRC(int data) {
    int tableIndex = (((getCrc() >> 8) & 0xFF) ^ data);
    if (tableIndex <= 0xff) {
      int tableValue = CRC.TABLE[tableIndex];
      setCrc(((getCrc() << 8) & 0xFF00) ^ tableValue);
    }
  }
  ...
  //          
  // for (int i = 0; i < data.length; i++) {
  // int tableIndex = (((crc >> 8) & 0xFF) ^ data[i]);
  // if (tableIndex <= 0xff) {
  // int tableValue = CRC.TABLE[tableIndex];
  // crc = ((crc << 8) & 0xFF00) ^ tableValue;
  // }
  // }
  // return crc;
  // }
  public UnsignedByte[] getUnsignedBytes() {
    UnsignedByte[] check = UnsignedByte.splitInt(crc, UnsignedByte.TWO_BYTES);
    UnsignedByte temp = check[0];
    check[0] = check[1];
    check[1] = temp;
    return check;
  }
