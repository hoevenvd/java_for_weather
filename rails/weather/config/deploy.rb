...
set :repository,  "svn+ssh://#{user}@tommitchell.net/home/#{user}/svn/#{application}/trunk/rails/#{application}"
...
task :symlink_public, :roles => :app do
  run "ln -nsf #{current_path}/public
       /home/tomorg/henrymitchell/weather/public_html"
end
...
task :chmod_files, :roles => :app do
  ...
  run "chmod +x #{release_path}/public/dispatch.cgi"
...
namespace(:deploy) do
  ...
  task :restart, :roles => :app do
    send(run_method, "")
    #send(run_method, "cd #{current_path} && mongrel_rails restart")    #run "#{current_path}/script/process/reaper --dispatcher=dispatch.fcgi"
  end
end
